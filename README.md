...
| `/library` | คลัง Quiz สาธารณะ | สาธารณะ |
| `/library/[id]` | รายละเอียด + โฮสต์/คัดลอก quiz สาธารณะ | สาธารณะ (โฮสต์/ก๊อปต้อง login) |
...
### คลัง Quiz สาธารณะ (V1 ใช้งานแล้ว)

- เจ้าของกดเผยแพร่จาก `/quizzes` หรือ `/quizzes/[id]/edit` (ต้องมี ≥1 ข้อ)
- หน้าคลัง `/library` ค้นหา + กรองหมวด/ความยาก + เรียงใหม่สุด/เล่นเยอะ/ก๊อปเยอะ
- หน้า `/library/[id]` ดูตัวอย่างคำถามแบบซ่อนเฉลย แล้วกดโฮสต์เลยหรือคัดลอกมาแก้
- API: `PATCH /api/quizzes/[id]/publish`, `GET /api/quizzes/public`, `GET /api/quizzes/public/[id]`, `POST /api/quizzes/[id]/fork`
- `POST /api/games` อนุญาตให้ host quiz ที่ `isPublished=true` ได้ พร้อมนับ `playCount`
